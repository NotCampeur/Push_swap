...
int					parse_args(int argc, char *argv[], t_bool *debug)
{
	if (argc == 1
		...
		|| (argc == 3 && (ft_strlen(argv[2]) == 0
				|| (argv[2][0] == '-' && ft_is_digit(argv[2][1]) == false))))
		return (EXIT_FAILURE);
	if (ft_strcmp(argv[1], "-v") == true)
		*debug = true;
	if (argc > (2 + (int)*debug))
	{
	...
	if (is_duplicate(argv[1 + *debug]) == true)
		return (EXIT_FAILURE);
	if (is_only_int(argv[1 + *debug]) == false)
		return (EXIT_FAILURE);
	if (is_suitable(argv[1 + *debug]) == false)
		return (EXIT_FAILURE);
	return (EXIT_SUCCESS);
}
